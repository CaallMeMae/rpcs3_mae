...
#include "Emu/GUI/modern/ModernManagerWindow.h"
#endif
...
void main_window::BootElf()
{
	...
	const QString file_path = QFileDialog::getOpenFileName(this, tr("Select (S)ELF To Boot"), path_last_elf, tr("(S)ELF files (*BOOT.BIN *.elf *.self);;"
...
void main_window::BootSavestate()
{
	...
	const QString file_path = QFileDialog::getOpenFileName(this, tr("Select Savestate To Boot"), QString::fromStdString(fs::get_config_dir() + "savestates/"), tr("Savestate files (*.SAVESTAT *.SAVESTAT.zst *.SAVESTAT.gz);;"
																																								  "All files (*.*)"),
		Q_NULLPTR, QFileDialog::DontResolveSymlinks);
...
#ifdef ENABLE_MODERN_MANAGER
	m_modern_manager_act = ui->menuView->addAction(tr("Modern Game Manager (Beta)"));
#endif
}
...
#ifdef ENABLE_MODERN_MANAGER
	connect(m_modern_manager_act, &QAction::triggered, this, [this]()
		{
			if (!m_modern_manager_window)
			{
				m_modern_manager_window = std::make_unique<rpcs3::ui::ModernManagerWindow>(this);
			...
			m_modern_manager_window->show();
			m_modern_manager_window->raise();
			m_modern_manager_window->activateWindow();
		});
#endif
	...
	connect(ui->menuGame_Categories, &QMenu::aboutToShow, ui->menuGame_Categories, [this, get_cats]()
		{
			const auto set_cat_count = [&](QAction* act, const QString& text)
			{
				...
				{
					if (game && categories.contains(QString::fromStdString(game->info.category)))
						count++;
				}
